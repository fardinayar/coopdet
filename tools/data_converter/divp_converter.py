...
class DIVP2NuScenes(object):
    """DIVP dataset to nuScenes converter.
    ...
    def __init__(self,
        ...
        self.vehicle_point_cloud_save_dir = f'vehicle'
        self.infrastructure_point_cloud_save_dir = f'rsu'
        self.registered_point_cloud_save_dir = f'rsu_lidar_combined'
    ...
    def convert(self):
        ...
        for split in self.splits:
            ...
            infrastructure_pcd_file_paths_sorted = sorted(glob(
                os.path.join(self.load_dir, self.map_version_to_dir[split], 'point_clouds', 'rsu',
                             '*')))
            ...
            if test:
                ...
                info_path = os.path.join(self.save_dir, "{}_infos_test.pkl".format('divp_nusc'))
                dump(data, info_path)
            else:
                if split == 'training':
                    print("train sample: {}".format(len(infos_list)))
                    data = dict(infos=infos_list, metadata=metadata)
                    info_path = os.path.join(self.save_dir, "{}_infos_train.pkl".format('divp_nusc'))
                    dump(data, info_path)
                elif split == 'validation':
                    print("val sample: {}".format(len(infos_list)))
                    data = dict(infos=infos_list, metadata=metadata)
                    info_path = os.path.join(self.save_dir, "{}_infos_val.pkl".format('divp_nusc'))
                    dump(data, info_path)

        print('\nFinished ...')

    def _fill_infos(self, vehicle_pcd_list, infrastructure_pcd_list, registered_pcd_list, pcd_labels_list, split):
        infos_list = []
        ...
        for i, pcd_path in enumerate(infrastructure_pcd_list):
            ...
            info = {
                ...
                "vehicle2infrastructure": lidar_anno_frame['frame_properties']['transforms'][
                    'vehicle_lidar_to_rsu_lidar']['transform_src_to_dst']['matrix4x4'],
                "lidar_anno_path": pcd_labels_list[i],
                "vehicle_cams": self._modify_image_data_paths(lidar_anno_frame['frame_properties']['vehicle_cams'], split),
                "infrastructure_cams": self._modify_image_data_paths(lidar_anno_frame['frame_properties']['infrastructure_cams'], split),
            }
    ...
    def _modify_image_data_paths(self, cams, split):
        new_cams = {}
        for camera_id, info in cams.items():
            new_info = info.copy()
            new_info["data_path"] =  os.path.join(self.load_dir, self.map_version_to_dir[split], 'images', info["data_path"])
            new_cams[camera_id] = new_info
        return new_cams
    ...
    def create_folder(self, split):
        ...
        self.registered_point_cloud_save_dir = os.path.join(self.save_dir, split_path,
                                                            f'point_clouds/rsu_lidar_combined')
